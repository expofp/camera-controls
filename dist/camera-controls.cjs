...
const SMOOTH_TIME_KEYS = ['rotateAzimuth', 'rotatePolar', 'truck', 'dolly', 'zoom', 'offset'];
const isMac$1 = /Mac/.test(globalThis?.navigator?.platform);
let THREE;
function getInstalledTHREE() {
    return THREE;
}
let _ORIGIN;
let _AXIS_Y;
...
class CameraControls extends EventDispatcher {
    ...
    constructor(camera, domElement) {
        ...
        const onMouseWheel = (event) => {
            ...
            const deltaYFactor = isMac$1 ? -1 : -3;
        ...
        const startDragging = (event) => {
            ...
            // - rotate
            if ((this._state & (ACTION.ROTATE | ACTION.TOUCH_ROTATE)) !== 0) {
            ...
            if ((this._state & (ACTION.TRUCK | ACTION.SCREEN_PAN | ACTION.TOUCH_TRUCK | ACTION.TOUCH_SCREEN_PAN)) !== 0) {
                this._targetEnd.copy(this._target);
                this._targetVelocity.set(0, 0, 0);
            }
            // - dolly
            if ((this._state & (ACTION.DOLLY | ACTION.TOUCH_DOLLY)) !== 0) {
                this._sphericalEnd.radius = this._spherical.radius;
                this._radiusVelocity.value = 0;
            }
            // - zoom
            if ((this._state & (ACTION.ZOOM | ACTION.TOUCH_ZOOM)) !== 0) {
                this._zoomEnd = this._zoom;
                this._zoomVelocity.value = 0;
            }
            // - offset
            if ((this._state & (ACTION.OFFSET | ACTION.TOUCH_OFFSET)) !== 0) {
        ...
        const dragging = () => {
            ...
            // touch dolly or zoom
            if ((this._state & (ACTION.TOUCH_DOLLY | ACTION.TOUCH_ZOOM)) !== 0) {
                const dx = _v2.x - this._activePointers[1].clientX;
                const dy = _v2.y - this._activePointers[1].clientY;
                const distance = Math.sqrt(dx * dx + dy * dy);
                ...
                if ((this._state & ACTION.TOUCH_DOLLY) !== 0) {
            ...
            // truck
            if ((this._state & (ACTION.TRUCK | ACTION.TOUCH_TRUCK)) !== 0) {
                this._truckInternal(deltaX, deltaY, false, false);
                this._isUserControllingTruck = true;
            }
            // screen-pan
            if ((this._state & (ACTION.SCREEN_PAN | ACTION.TOUCH_SCREEN_PAN)) !== 0) {
    ...
    _rotateInternal = (deltaX, deltaY, state) => {
    ...
    _applySmoothTime(target, value) {
        ...
        if (typeof value === 'number') {
            ...
            if (value.rotate !== undefined)
                target.rotateAzimuth = target.rotatePolar = value.rotate;
 ...
 */
function intersectRayPlane(ox, oy, oz, dx, dy, dz, nx, ny, nz, planeConstant) {
 ...
 */
class MapControls extends CameraControls {
    /**
    ...
    constructor(camera, domElement) {
        ...
        this._dollyInternal = (delta, x, y) => {
            ...
            if (this.dollyToCursor && !this.infinityDolly && isPerspectiveCamera(camera)) {
                ...
                if (forwardDotN > -GRAZE_EPSILON) {
                    ...
                    if (right.lengthSq() < GRAZE_EPSILON)
                    ...
                    if (bearing.lengthSq() > GRAZE_EPSILON)
                        bearing.normalize();
        ...
        this._zoomInternal = (delta, x, y) => {
            ...
            if (this.dollyToCursor && !this.infinityDolly && isOrthographicCamera(camera)) {
                ...
                const shift = this._truckShift;
                if (w >= 1)
                    shift.copy(anchorShift);
                else if (w <= 0)
                    shift.copy(truckShift);
                else
                    shift.copy(truckShift).lerp(anchorShift, w);
                const newTarget = this._newTarget.copy(this._targetEnd).add(shift);
                this._boundary.clampPoint(newTarget, newTarget);
